...
local vthIon = math.sqrt(TIon/massIon)

local nuElc = 1.0e-4
local nuElcIon = 1.0e-4
local nuIon = nuElc/math.sqrt(massIon)
local nuIonElc = nuElcIon/massIon
...
vlasovApp = Plasma.App {
   ...
   ion = Plasma.Species {
      ...
      init = function (t, xn)
         local x, y, vx, vy = xn[1], xn[2], xn[3], xn[4]
         local fv = maxwellian2D(nIon, vx, vy, 0.0, 0.0, vthIon)
         return fv
         end,
      evolve = true,
      diagnosticMoments = {"M0", "M1i", "M2ij", "M3i" },
